...
import type { AccountDeposit, UpdateAccountDepositDTO } from '../../models/AccountDeposit';
import type { Account } from '../../models/Account';

interface AccountDepositUpdateProps {
  deposit: AccountDeposit;
  onClose: () => void;
  onSuccess: () => void;
}

export function AccountDepositUpdate({ deposit, onClose, onSuccess }: AccountDepositUpdateProps) {
  ...
  const [formData, setFormData] = useState<UpdateAccountDepositDTO>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await accountDepositServices.updateDeposit(deposit.id, formData);
      onSuccess();
    } catch (error) {
      console.error('Error updating deposit:', error);
      setErrors({ submit: t('messages.updateDepositFailed') });
    } finally {
      setSubmitting(false);
    }
  };

  const handleInputChange = (field: keyof UpdateAccountDepositDTO, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            {t('accountDeposit.editDeposit', { id: deposit.id })}
          </h3>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <Button type="submit" disabled={submitting} color="success">
              {submitting ? (
                ...
                t('accountDeposit.updateDeposit')
              )}
            </Button>
          </div>
        </form>
